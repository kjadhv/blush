...
const formatDate = (exp) => {
  // Support both real Firestore timestamp and plain ms number
  let ms;
  if (exp.createdAtMs)            ms = exp.createdAtMs;
  else if (exp.createdAt?.seconds) ms = exp.createdAt.seconds * 1000;
  else return "";
      ...
      <div style={S.content}>
        {loading ? (
          ...
          <div style={S.emptyWrap}>
            ...
            <button style={S.createBtn} onClick={() => setShowCreate(true)}>+ Create Group</button>
          </div>
        ) : (
          groups.map((g, i) => (
            <GroupCard
  ...
  return (
    <div style={{ ...S.groupCard, animationDelay: `${index * 0.07}s` }} className="group-card" onClick={onClick}>
      ...
      <div style={S.groupCats}>
        ...
        {(g.expenses || []).length === 0 && <span style={S.noExpYet}>No expenses yet — tap to add</span>}
      </div>
...
function GroupDetail({ group, user, onBack, onDelete, onUpdate }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const aMs = a.createdAtMs || (a.createdAt?.seconds * 1000) || 0;
    const bMs = b.createdAtMs || (b.createdAt?.seconds * 1000) || 0;
    return bMs - aMs;
  });
  ...
  const handleAddExpense = async (expense) => {
    try {
      ...
      const newExp = {
        ...
        createdAtMs: now,   // ← store as plain ms number — reliable across devices
      };
      const updated  = [...expenses, newExp];
      const groupRef = doc(db, "expenseGroups", group.id);
      // Save full updated array — more reliable than arrayUnion for complex objects
  ...
  // ── Fixed delete — filter by id then save whole array ──────────
  const handleDeleteExpense = async (expId) => {
    if (!window.confirm("Delete this expense?")) return;
    try {
      const updated  = expenses.filter(e => e.id !== expId);
      const groupRef = doc(db, "expenseGroups", group.id);
      await updateDoc(groupRef, { expenses: updated }); // save full array
      setExpenses(updated);
      onUpdate({ ...group, expenses: updated });
    } catch (e) {
      console.error(e);
    ...
    <div style={S.root}>
      ...
      {usedCats.length > 0 && (
        <div style={S.filterScroll}>
          <button style={{ ...S.filterPill, ...(filterCat === "all" ? S.filterActive : {}) }}
          ...
          {usedCats.map(cat => {
            ...
            return (
              <button key={cat}
                style={{ ...S.filterPill, ...(filterCat === cat ? { background: preset.color, color: "#fff", borderColor: preset.color } : {}) }}
                onClick={() => setFilterCat(cat)}>
                {preset.emoji} {preset.label} ({expenses.filter(e => e.category === cat).length})
              </button>
            );
          })}
        </div>
              ...
              <div key={exp.id} style={{ ...S.expCard, animationDelay: `${i * 0.04}s` }} className="exp-card">
                ...
                <div style={S.expBody}>
                  ...
                  <div style={S.expMeta}>
                    ...
                    <span style={S.expWho}>{isMe ? "you" : exp.paidByName || "her"} paid</span>
                  </div>
                  {/* ── Date + time ── */}
                  <p style={S.expDate}>
                    {formatDate(exp)} {formatTime(exp) && `· ${formatTime(exp)}`}
                  </p>
                </div>
                <div style={S.expRight}>
                  ...
                  {isMe && (
                    <button style={S.expDelete} onClick={() => handleDeleteExpense(exp.id)}>🗑</button>
                  )}
                </div>
              </div>
            );
      ...
      {showAdd && (
        <AddExpenseSheet onClose={() => setShowAdd(false)} onAdd={handleAddExpense} />
      )}
...
function CreateGroupSheet({ user, onClose, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    } catch (e) { console.error(e); setErr("Failed to create. Try again."); }
    setSaving(false);
  };
    ...
    <div style={S.sheetOverlay} onClick={onClose}>
      <div style={S.sheet} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.sheetScroll}>
          ...
          <input style={S.input} placeholder="e.g. Goa Trip 🌊" value={name} onChange={e => setName(e.target.value)} />
          <label style={S.label}>Sister's Email *</label>
          <input style={S.input} type="email" placeholder="her@email.com" value={email} onChange={e => setEmail(e.target.value)} />
          <p style={S.inputHint}>She must already have an account on this app</p>
          {err && <p style={S.errTxt}>{err}</p>}
          <button style={{ ...S.saveBtn, opacity: saving ? 0.6 : 1 }} onClick={handleCreate} disabled={saving}>
      ...
      <div style={S.sheet} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.sheetScroll}>
          ...
          <div style={S.amountWrap}>
            ...
            <input style={S.amountInput} type="number" placeholder="0" value={amount}
              onChange={e => setAmount(e.target.value)} autoFocus />
          </div>
          <label style={S.label}>Note (optional)</label>
          <input style={S.input} placeholder="e.g. Chai at Starbucks" value={note} onChange={e => setNote(e.target.value)} />
          <label style={S.label}>Category *</label>
          <div style={S.catGrid}>
            {PRESET_CATEGORIES.map(cat => (
              <button key={cat.id}
                style={{ ...S.catGridBtn, background: category === cat.id ? cat.color : cat.color + "15", color: category === cat.id ? "#fff" : cat.color, border: `1.5px solid ${category === cat.id ? cat.color : cat.color + "44"}` }}
            ...
            <button
              style={{ ...S.catGridBtn, background: category === "__custom__" ? "#6366f1" : "#6366f115", color: category === "__custom__" ? "#fff" : "#6366f1", border: `1.5px solid ${category === "__custom__" ? "#6366f1" : "#6366f144"}` }}
          ...
          {category === "__custom__" && (
            <input style={{ ...S.input, marginTop: 10 }} placeholder="Type your category"
...
const S = {
  ...
  expMeta:       { display:"flex", gap:8, alignItems:"center", marginBottom:3 },
  expCatTag:     { fontSize:11, fontWeight:600, borderRadius:6, padding:"2px 8px" },
  expWho:        { fontSize:11, color:"#f9a8d4" },
  expDate:       { fontSize:11, color:"#ccc" },   // ← date style
